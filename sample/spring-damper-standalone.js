...
const EPSILON_PREDICTOR_V3_STIFFNESS = 0.01 ;
...
async function updatePredictorV3( dt ) {
	...
	if ( predictedAcceleration !== initialAcceleration ) {
		...
		if ( predictedAcceleration * correctedAcceleration < 0 && Math.abs( predictedAcceleration - correctedAcceleration ) > EPSILON_PREDICTOR_V3_STIFFNESS ) {
...
function tracerReport() {
	...
	var analyticFn = math.fn.Const2ndOrdDifferentialEquationFn.createSpringDamperMass(
		springK , dampingFactor , mass , initialDisplacement , initialVelocity , externalForce
	) ;
	...
	tracer.saveImage( __dirname + "/spring-damper-standalone.png" ) ;
}
